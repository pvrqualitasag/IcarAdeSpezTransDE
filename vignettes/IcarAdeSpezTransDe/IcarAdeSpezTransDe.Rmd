...
output:
  pdf_document:
    ...
    keep_tex: yes
    fig_caption: false
bibliography: IcarAdeSpezTransDe.bibtex
---
...
{Die folgende Übersicht zum Thema Web Service ist nicht Teil der ursprünglichen Spezifikation
...
```
Vorläufer von Webservices, die da hiessen RPC (remote procedure call) oder RMI (remote method invocation) hatten den oben formulierten Wunsch schon realisiert. Diese Ansätze waren aber wenig flexibel. Schliesslich wurden sie durch Web services abgelöst. 

```{r echo=FALSE,results='asis'}
robjSecEnum$displayNumSection("## Architektur")
```
Webservices orientieren sich an der sogenannten serviceorientierten Architektur (SOA). Der Begriff SOA ist nicht einheitlich definiert. Eine mögliche Definition, welche aus @Melzer_2010 entnommen wurde, lautet: "Unter einer SOA versteht man eine Systemarchitektur, die vielfältige und eventuell inkompatible Methoden oder Applikationen als wiederverwendbare und offen zugreifbare Dienste repräsentiert und dadurch eine plattform- und sprachunabhängige Nutzung und Wiederverwendung ermöglicht." 

}

```{r WriteCitations, eval=FALSE, echo=FALSE, results='hide', warning=FALSE}
sBibFile <- "IcarAdeSpezTransDe.bibtex"
